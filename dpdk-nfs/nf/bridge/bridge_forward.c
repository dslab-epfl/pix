...
int bridge_expire_entries(time_t time) {
  ...
  uint64_t min_time_u =
      time_u -
      config.expiration_time; // OK because time >= expiration_time >= 0
...
void bridge_put_update_entry(struct ether_addr *src, uint16_t src_device,
  ...
  if (present) {
    ...
    int allocated = dchain_allocate_new_index(dynamic_ft.heap, &index, time);
    if (!allocated) {
...
void allocate_static_ft(int capacity) {
  ...
  int happy =
      map_allocate(static_key_eq, static_key_hash, capacity, &static_ft.map);

  if (!happy)
    rte_exit(EXIT_FAILURE, "error allocating static map");
  happy = vector_allocate(sizeof(struct StaticKey), capacity, init_nothing_st,
                          &static_ft.keys);
  if (!happy)
...
struct str_field_descr dynamic_vector_key_fields[] = {
    {0, sizeof(uint8_t), "a"}, {1, sizeof(uint8_t), "b"},
    {2, sizeof(uint8_t), "c"}, {3, sizeof(uint8_t), "d"},
    {4, sizeof(uint8_t), "e"}, {5, sizeof(uint8_t), "f"},
};
...
int stat_map_condition(void *key, int index) {
  ...
  return 0 <= index & index < rte_eth_dev_count_avail();
}
...
void nf_core_init(void) {
  ...
  happy = vector_allocate(sizeof(struct DynamicValue), capacity,
                          init_nothing_dv, &dynamic_ft.values);
  if (!happy)
...
int nf_core_process(struct rte_mbuf *mbuf, time_t now) {
  ...
  int dst_device = bridge_get_device(&ether_header->d_addr, mbuf->port);
  ...
  klee_assume(dst_device < rte_eth_dev_count_avail());
  for (unsigned d = 0; d < rte_eth_dev_count_avail(); d++)
    if (dst_device == d) {
      dst_device = d;
      break;
    }
...
void nf_print_config() { bridge_print_config(&config); }

#ifdef KLEE_VERIFICATION

void nf_loop_iteration_border(unsigned lcore_id, time_t time) {
  bridge_loop_iteration_border(&dynamic_ft.heap, &dynamic_ft.map,
                               &dynamic_ft.keys, &dynamic_ft.values,
                               &static_ft.map, &static_ft.keys,
                               config.dyn_capacity, time, rte_eth_dev_count_avail());
}
