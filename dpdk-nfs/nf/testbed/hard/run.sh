...
case $SCENARIO in
	...
	case $NF_TYPE in
        ...
        ssh $TESTER_HOST "sudo ~/moon-gen/build/MoonGen ~/scripts/moongen/$LUA_SCRIPT -r 1000 -u 5 -t 10 1 0"
        scp $TESTER_HOST:mf-lat.txt "./$RESULTS_FILE"
        ssh $TESTER_HOST "sudo rm mf-lat.txt"
        ;;

    "replay-pcap"|"replay-pcap-instr")
        LUA_SCRIPT="replay-pcap.lua"
        echo "[bench] Benchmarking latency..."
        ssh $TESTER_HOST "sudo ~/moon-gen/build/MoonGen ~/scripts/moongen/$LUA_SCRIPT -r 1000 -t 10 -f $PCAP_FILE 1 0"
        scp $TESTER_HOST:pcap-replay-results.txt "./$RESULTS_FILE"
        ssh $TESTER_HOST "sudo rm pcap-replay-results.txt"
        ;;
