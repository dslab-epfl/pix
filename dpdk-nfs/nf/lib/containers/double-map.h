...
typedef void
    uq_value_copy /*@<K>(predicate (void*; K) vp, int size) @*/ (char *dst,
                                                                 void *src);
...
/*@ requires [?fr]bare_valp(vp, ?v) &*& [fr]keyp1(kp1, ?k1) &*& [fr]keyp2(kp2,
   ?k2) &*& true == right_offsets(vp, kp1, kp2) &*& k1 == vk1(v) &*& k2 ==
   vk2(v); @*/
...
/*@
  ...
  fixpoint bool dmap_self_consistent_integral_fp<t1,t2,vt>(dmap<t1,t2,vt> m,
                                                           fixpoint (vt,t1) vk1,
                                                           fixpoint (vt,t2) vk2)
  { switch(m) { case dmap(m1, m2, vals): return dmap_self_consistent_fp(vals,
  m1, m2, vk1, vk2, 0) && true == no_dups(map(fst, m1)) && true ==
  no_dups(map(fst, m2));
    }
  }
  ...
  ensures dmappingp<t1,t2,vt>(m,
          ...
          true == recp2(vk2(dmap_get_val_fp(m, dmap_get_k1_fp(m, k))),
  dmap_get_k1_fp(m,k)) &*& true == recp1(k, dmap_get_k1_fp(m,k));
  ...
  ensures dmappingp<t1,t2,vt>(m,
          ...
          true == recp1(vk1(dmap_get_val_fp(m, dmap_get_k2_fp(m, k))),
  dmap_get_k2_fp(m,k)) &*& true == recp2(k, dmap_get_k2_fp(m,k));
...
int dmap_allocate /*@ <K1,K2,V> @*/
    (map_keys_equality *eq_a, map_key_hash *hsh_a, map_keys_equality *eq_b,
     ...
     int capacity, struct DoubleMap **map_out);
/*@ requires dmap_key_val_types<K1,K2,V>(_, _, _) &*&
   ...
   &*& dmap_record_property1<K1>(?recp1) &*& dmap_record_property2<K2>(?recp2)
   &*& *map_out |-> ?old_map_out &*& 0 < value_size &*& value_size < 4096 &*& 0
   < capacity &*& capacity <= keys_capacity &*& keys_capacity <
