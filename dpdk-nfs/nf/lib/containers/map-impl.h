...
typedef bool map_keys_equality /*@<K>(predicate (void*; K) keyp) @*/ (void *k1,
                                                                      void *k2);
//@ requires [?fr1]keyp(k1, ?kk1) &*& [?fr2]keyp(k2, ?kk2);
//@ ensures [fr1]keyp(k1, kk1) &*& [fr2]keyp(k2, kk2) &*& (false == result ?
//(kk1 != kk2) : (kk1 == kk2));
...
/*@
  ...
  { switch(m) { case nil: return nil; case cons(h,t): return fst(h) == key ? t :
  cons(h, map_erase_fp(t, key));
    }
  }

  fixpoint list<pair<kt,vt> > map_erase_all_fp<kt,vt>(list<pair<kt,vt> > m,
  list<kt> keys) { switch(keys) { case nil: return m; case cons(h,t): return
  map_erase_fp(map_erase_all_fp(m, t), h);
    }
  }
