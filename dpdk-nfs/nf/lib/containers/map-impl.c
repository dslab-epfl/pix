...
/*@
  ...
  &*& loop_fp(hash(k), capacity) < shift &*& shift <= capacity &*& capacity -
  shift == length(buckets); ensures false == exists(buckets,
  (bucket_has_key_fp)(k));
  {
    switch(buckets) {
      case nil: return;
      case cons(bh,bt):
        switch(bh) { case bucket(chains):
          if (bucket_has_key_fp(k, bh)) {
              ...
              assert true == forall(chains, (has_given_hash_fp)(hash, shift,
  capacity)); hash_for_given_key(chains, hash, shift, capacity, k); assert true
...
/*@
  ...
  hash) &*& shift != loop_fp(hash(k), capacity); ensures false ==
  bucket_has_key_fp(k, bh);
  {
    switch(bh) { case bucket(chains):
...
/*@
  lemma void key_is_contained_in_the_bucket_rec<kt>(list<bucket<kt> > buckets,
                                                    ...
                                                    kt k)
  requires true == key_chains_start_on_hash_fp(buckets, shift, capacity, hash)
  &*& true == buckets_ok_rec(acc, buckets, capacity) &*& false == mem(k,
  map(fst, acc)) &*& 0 <= shift &*& shift <= loop_fp(hash(k), capacity) &*& 0 <
  capacity &*& capacity - shift == length(buckets) &*& false == mem(k, map(fst,
  get_wraparound(acc, buckets))) &*& buckets != nil; ensures loop_fp(hash(k),
  capacity) - shift < length(buckets) &*& mem(some(k),
  buckets_get_keys_rec_fp(acc, buckets)) == bucket_has_key_fp(k,
  nth(loop_fp(hash(k), capacity) - shift, buckets));
  {
    switch(buckets) {
      case nil: return;
      case cons(bh,bt):
  ...
  length(buckets)); if (loop_fp(hash(k), capacity) == shift) { if (mem(some(k),
  buckets_get_keys_rec_fp(acc, buckets))) { some_bucket_contains_key_rec(acc,
  buckets, k); switch(bh) { case bucket(chains): } overshoot_bucket(bt, shift+1,
  capacity, hash, k); if (bucket_has_key_fp(k, nth(loop_fp(hash(k), capacity) -
  shift, buckets))) { } else { assert false;
            }
          } else {
            if (bucket_has_key_fp(k, nth(loop_fp(hash(k), capacity) - shift,
  buckets))) { in_this_bucket_then_in_the_map(buckets, loop_fp(hash(k),
  capacity) - shift, k, capacity, acc); assert false; } else {
          ...
          assert true == (bucket_has_key_fp(k, nth(loop_fp(hash(k), capacity) -
  shift, buckets)) == bucket_has_key_fp(k, nth(loop_fp(hash(k), capacity) -
  shift - 1, bt)));
        }
    }
  }
...
/*@
  ...
  requires nth(i, hmap_ks_fp(m)) == some(k) &*& true ==
opt_no_dups(hmap_ks_fp(m)) &*& 0 <= i &*& i < length(hmap_ks_fp(m)); ensures
hmap_find_key_fp(m, k) == i;
  {
...
static int find_key /*@ <kt> @*/ (int *busybits, void **keyps, int *k_hashes,
                                  int *chns, void *keyp, map_keys_equality *eq,
                                  int key_hash, int capacity)
             ...
             (result == -1)); @*/
{
  ...
  //@ decreases capacity - i;
  {
    ...
    if (bb != 0 && kh == key_hash) {
...
#ifdef DUMP_PERF_VARS
        NF_PERF_DEBUG("t:%d",buckets_traversed);
        NF_PERF_DEBUG("c:%d",hash_collisions);
#endif
        return index;
      }
#ifdef DUMP_PERF_VARS
      ...
      if (chn == 0) {
        ...
        //@ assert true == up_to(nat_of_int(i), (byLoopNthProp)(ks,
        //(neq)(some(k)), capacity, start));
        //@ assert true == up_to(nat_of_int(i), (byLoopNthProp)(ks,
        //(neq)(some(k)), capacity, loop_fp(hsh(k), capacity)));
        //@ assert true == up_to(succ(nat_of_int(i)), (byLoopNthProp)(ks,
        //(neq)(some(k)), capacity, loop_fp(hsh(k), capacity)));
        //@ assert true == up_to(nat_of_int(i+1), (byLoopNthProp)(ks,
        //(neq)(some(k)), capacity, loop_fp(hsh(k), capacity)));
        //@ assert buckets != nil;
  ...
  //@ pred_mapping_same_len(bbs, ks);
  //@ by_loop_for_all(ks, (neq)(some(k)), start, capacity,
  // nat_of_int(capacity));
  //@ no_key_found(ks, k);
  //@ close buckets_ks_insync(chns, capacity, buckets, hsh, ks);
  //@ close hmapping<kt>(kpr, hsh, capacity, busybits, kps, k_hashes, hm);
...
/*@
  ...
  requires pred_mapping(kps, bbs, ?keyp, ks) &*&
           ...
           nth(index, ks) == some(?k);
  ensures pred_mapping(kps, update(index, 0, bbs), keyp, update(index, none,
  ks)) &*& [0.5]keyp(nth(index, kps), k);
  {
    ...
    close pred_mapping(kps, update(index, 0, bbs), keyp, update(index, none,
  ks));
  }//took 5m
  @*/
...
static int find_key_remove_chain /*@ <kt> @*/ (
    int *busybits, void **keyps, int *k_hashes, int *chns, void *keyp,
    map_keys_equality *eq, int key_hash, int capacity, void **keyp_out)
/*@ requires hmapping<kt>(?kpr, ?hsh, capacity, busybits, ?kps, k_hashes, ?hm)
   &*& buckets_ks_insync(chns, capacity, ?buckets, hsh, hmap_ks_fp(hm)) &*&
             pointers(keyps, capacity, kps) &*&
             [?kfr]kpr(keyp, ?k) &*&
            ...
            [0.5]kpr(ptr, k); @*/
{
  ...
  //@ decreases capacity - i;
  {
    ...
    //@ assert true == (loop_fp(loop_fp(start + i, capacity) + 1, capacity) ==
...
/*@
  ...
  lemma void bb_nonzero_cell_busy<kt>(list<int> bbs, list<option<kt> > ks, int
  i) requires pred_mapping(?kps, bbs, ?kp, ks) &*& nth(i, bbs) != 0 &*& 0 <= i
  &*& i < length(bbs); ensures pred_mapping(kps, bbs, kp, ks) &*& true ==
  cell_busy(nth(i, ks));
  {
...
/*@
                                   ...
                                   start, start, ks);
  {
    ...
    close buckets_ks_insync_Xchain(chns, capacity, buckets, hsh, start, start,
  ks);
  }//took 5m
  @*/
...
ensures false == bucket_has_key_fp(k, nth(start, buckets));
{
  if (bucket_has_key_fp(k, nth(start, buckets))) {
    ...
    assert true == mem(k, map(fst, get_crossing_chains_fp(buckets, loop_fp(start
+ i, capacity)))); loop_lims(start + i, capacity);
...
static int find_empty /*@ <kt> @*/ (int *busybits, int *chns, int start,
                                    int capacity)
          ...
          nat_of_int(length(cons(h, t))) == succ(nat_of_int(length(t)));
  {
    ...
    switch(nat_of_int(l)) {
      ...
      case succ(lll):
        ...
        assert(nat_of_int(int_of_nat(lll) + 1) ==
  succ(nat_of_int(int_of_nat(lll)))); assert(nat_of_int(int_of_nat(lll) + 1) ==
  nat_of_int(l)); assert(int_of_nat(nat_of_int(int_of_nat(lll) + 1)) ==
  int_of_nat(nat_of_int(l))); assert(int_of_nat(lll) + 1 == l);
...
void map_impl_init /*@ <kt> @*/ (int *busybits, map_keys_equality *eq,
                                 void **keyps, int *khs, int *chns, int *vals,
                                 int capacity)
            ...
            [fr]is_map_keys_equality<kt>(eq, keyp); @*/
{
  ...
  //@ close hmapping<kt>(keyp, hash, capacity, busybits, kps, khs,
  // empty_hmap_fp<kt>(capacity, khlist));
...
int map_impl_get /*@ <kt> @*/ (int *busybits, void **keyps, int *k_hashes,
                               int *chns, int *values, void *keyp,
                               map_keys_equality *eq, int hash, int *value,
                               int capacity)
              ...
              *value |-> v)); @*/
{
  ...
  //@ open mapping(m, addrs, kp, recp, hsh, capacity, busybits, keyps, k_hashes,
  // chns, values);
  ...
  if (-1 == index) {
    ...
    ds_path_1();
#endif
    //@ close mapping(m, addrs, kp, recp, hsh, capacity, busybits, keyps,
    // k_hashes, chns, values);
    return 0;
  }
#ifdef REPLAY_BR
  ds_path_2();
    ...
    switch(ks) {
      ...
      case cons(h,t):
        ...
        if (index == 0) {
          ...
          assert(head(update(0, nhsh, khs)) == hash(get_some(head(update(0,
  some(nk), ks))))); } else { hmap_coherent_hash_update(t, tail(khs), hash,
  index-1, nk, nhsh); cons_head_tail(khs); update_tail_tail_update(head(khs),
  tail(khs), index, nhsh); update_tail_tail_update(h, t, index, some(nk));
        }
    }
  }
  @*/
      ...
      case cons(h,t):
        if (i == 0) {
          ...
          if (h == none) {
            ...
            assert(true == mem(pair(get_some(h),head(vals)), map_put_fp(m, k,
  v)));
            ...
            assert(map_put_fp(remove(pair(get_some(h), head(vals)), m), k, v) ==
                   remove(pair(get_some(h), head(vals)), (map_put_fp(m, k,
  v))));
          }
        }
...
void map_impl_put /*@ <kt> @*/ (int *busybits, void **keyps, int *k_hashes,
                                int *chns, int *values, void *keyp, int hash,
                        ...
                        keyps, k_hashes, chns, values); @*/
{
  ...
  //@ coherent_put_preserves_key_vals(hmap_ks_fp(hm), kps, addrs, index, k,
  // keyp);
  ...
  /*@ close mapping(map_put_fp(m, k, value), map_put_fp(addrs, k, keyp),
                    kp, recp, hsh, capacity, busybits, keyps, k_hashes, chns,
    values);
    @*/
}
...
void map_impl_erase /*@ <kt> @*/ (int *busybits, void **keyps, int *k_hashes,
                                  int *chns, void *keyp, map_keys_equality *eq,
                                  int hash, int capacity, void **keyp_out)
/*@ requires mapping<kt>(?m, ?addrs, ?kp, ?recp, ?hsh, capacity, busybits,
                         keyps, k_hashes, chns, ?values) &*&
             [0.5]kp(keyp, ?k) &*&
   ...
   @*/
{
  ...
  // @ hmap_rem_preserves_no_dups(ks, index);
  // @ hmap_rem_preserves_hash_list(ks, khs, hsh, index);
  // @ close hmapping(kp, hsh, capacity, busybits, kps, k_hashes,
  // hmap_rem_key_fp(hm, index));
