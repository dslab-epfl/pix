...
/*@
  lemma void glue_by_index_half<t>(char* data, int idx, list<pair<t, bool> >
  lst) requires 0 <= idx &*& idx < length(lst) &*& entsp<t>(data, ?el_size,
  ?entp, idx, take(idx, lst)) &*& nth(idx, lst) == pair(?val, false) &*&
   ...
   @*/
{
  ...
  //@ glue_by_index_full(vector->data, index, update(index, pair(v, true),
  // values));
