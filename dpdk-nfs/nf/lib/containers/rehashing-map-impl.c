...
int longest_collision_chain(int* chns, int* key_hashes, int capacity)
{
   int i,start,index;
   ...
   {
     index = (start+i)%capacity;
     if(chns[index]!=0 && key_hashes[index] < capacity)
     { 
        curr[key_hashes[index]]=curr[key_hashes[index]] +1;
        if(curr[key_hashes[index]] >max)
        {max = curr[key_hashes[index]];}
        else
        {
           if(max > longest)
           {
              ...
              *value |-> v)); @*/
{
  ...
  int longest_cchain = longest_collision_chain(chns,k_hashes,capacity);
                        ...
                        keyps, k_hashes, chns, values); @*/
{
  ...
  //@ close buckets_ks_insync(chns, capacity, buckets, hsh, ks);
  //@ assert length(buckets) == capacity;
  int index = find_empty(busybits, chns, start, capacity, num_traversals);
