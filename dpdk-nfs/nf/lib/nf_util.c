...
struct ipv4_hdr *nf_get_mbuf_ipv4_header(struct rte_mbuf *mbuf) {
  ...
  if (ether_header->ether_type != rte_cpu_to_be_16(ETHER_TYPE_IPv4)) {
    return NULL;
  }

  return rte_pktmbuf_mtod_offset(mbuf, struct ipv4_hdr *,
...
struct tcpudp_hdr *nf_get_ipv4_tcpudp_header(struct ipv4_hdr *header) {
  ...
  uint8_t offset =
      (header->version_ihl & IPV4_HDR_IHL_MASK) * IPV4_IHL_MULTIPLIER;
...
struct icmp_hdr *nf_get_ipv4_icmp_header(struct ipv4_hdr *header) {
  ...
  uint8_t offset =
      (header->version_ihl & IPV4_HDR_IHL_MASK) * IPV4_IHL_MULTIPLIER;
  return (struct icmp_hdr *)((unsigned char *)header + offset);
}

__attribute__((noinline)) void nf_set_ipv4_checksum(struct ipv4_hdr *header) {
  ...
  if (header->next_proto_id == IPPROTO_TCP) {
    ...
    struct icmp_hdr* icmp_header = (struct icmp_hdr *)(header+1);
