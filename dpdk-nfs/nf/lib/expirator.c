...
int expire_items /*@<K1,K2,V> @*/ (struct DoubleChain *chain,
                                   struct DoubleMap *map, time_t time)
            ...
            result == length(dchain_get_expired_indexes_fp(ch, time)); @*/
{
                  ...
                  count <= length(dchain_get_expired_indexes_fp(ch, time)); @*/
  {
    ...
    //@ coherent_expire_one(cur_m, cur_ch, dchain_get_oldest_index_fp(cur_ch),
    // vk1, vk2);
  }
...
/*@
  lemma void vector_erase_all_same_len<t>(list<pair<t, bool> > vec, list<int>
  indices) requires true; ensures length(vec) == length(vector_erase_all_fp(vec,
  indices));
  {
    assume(false);//TODO
  }
...
/*@ ensures mapp<kt>(map, kp, hsh, recp, mapc(cap, ?nm, ?naddrs)) &*&
   ...
   dchain_get_expired_indexes_fp(ch, time))) &*& naddrs ==
   map_erase_all_fp(addrs, vector_get_values_fp(v,
   dchain_get_expired_indexes_fp(ch, time))) &*& nv == vector_erase_all_fp(v,
   dchain_get_expired_indexes_fp(ch, time)) &*& map_vec_chain_coherent<kt>(nm,
   nv, nch) &*& length(nv) == length(v) &*& result ==
   length(dchain_get_expired_indexes_fp(ch, time)); @*/
{
      ...
      @*/
  {
    ...
    //@ map_erase_another_one(addrs, vector_get_values_fp(v, cur_exp_indices),
    // k);
  ...
  /*@ if(dchain_is_empty_fp(expire_n_indexes(ch, time, count))) {
    ...
    @*/
  //@ vector_erase_all_same_len(v, take(count, dchain_get_expired_indexes_fp(ch,
  // time)));
