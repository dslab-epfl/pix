// Taken from - https://github.com/dslab-epfl/tinynf/blob/master/code/util/perf.h.

// Performance measurement utility, with three functions:
// - TN_PERF_PAPI_INIT(), to call at the beginning
// - TN_PERF_PAPI_RESET(), to call before the measured event(s)
// - TN_PERF_PAPI_RECORD(batch_size), to call after the measured event(s), with batch_size indicating the number of events
...
static int papi_events[] = {PAPI_REF_CYC, PAPI_TOT_INS, PAPI_L1_ICM, PAPI_L1_DCM, PAPI_L2_TCM, PAPI_L3_TCM};
...
static long long papi_packet_counter;
	...
	for (size_t n = papi_heatup_count; n < TN_DEBUG_PERF + papi_heatup_count; n++) {
		...
		for (m = 0; m < papi_batches[n] && n + m < TN_DEBUG_PERF + papi_heatup_count; m++) {
			printf("%lf %lf %lf %lf %lf %lf\n",
				...
				((double) papi_values[n][2] - papi_heatup_averages[2]) / papi_batches[n],
				((double) papi_values[n][3] - papi_heatup_averages[3]) / papi_batches[n],
				((double) papi_values[n][4] - papi_heatup_averages[4]) / papi_batches[n],
...
#define TN_PERF_PAPI_LOG_PKT() do { \
	papi_packet_counter++; \
	if(papi_packet_counter >= PAPI_BATCH_SIZE){\
		TN_PERF_PAPI_RECORD(papi_packet_counter); \
		TN_PERF_PAPI_RESET(); \
	} \
} while (0)
#else
