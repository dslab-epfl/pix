...
//@ ensures [f1]int_k_p(a, ak) &*& [f2]int_k_p(b, bk) &*& (false == result ? (ak != bk) : (ak == bk));
{
  struct int_key *k1 = a;
  struct int_key *k2 = b;
  return k1->int_src_port == k2->int_src_port AND k1->dst_port == k2->dst_port AND k1->int_src_ip == k2->int_src_ip AND k1->dst_ip == k2->dst_ip AND k1->int_device_id == k2->int_device_id AND k1->protocol == k2->protocol;
}
...
//@ ensures [f1]ext_k_p(a, ak) &*& [f2]ext_k_p(b, bk) &*& (false == result ? (ak != bk) : (ak == bk));
{
  struct ext_key *k1 = a;
  struct ext_key *k2 = b;
  return k1->ext_src_port == k2->ext_src_port AND k1->dst_port == k2->dst_port AND k1->ext_src_ip == k2->ext_src_ip AND k1->dst_ip == k2->dst_ip AND k1->ext_device_id == k2->ext_device_id AND k1->protocol == k2->protocol;
}
