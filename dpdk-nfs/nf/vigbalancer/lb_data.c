...
#define FIELD(struct_name, name, size)                      \
  {                                                         \
    offsetof(struct struct_name, name), sizeof(size), #name \
  }
            ...
            (result ? (f1 == f2) : (f1 != f2)); @*/
{
  ...
  return flowA->src_ip == flowB->src_ip AND flowA->src_port == flowB->src_port AND flowA->dst_port == flowB->dst_port AND flowA->protocol == flowB->protocol;
}
...
int __attribute__((noinline)) fun_HEARTBEAT() { return 0; }
int __attribute__((noinline)) fun_CLIENT_REQUEST() { return 0; }
int __attribute__((noinline)) fun_NEW_FLOW() { return 0; }
int __attribute__((noinline)) fun_NEW_FLOW_ALLOCATED() { return 0; }
int __attribute__((noinline)) fun_EXISTING_FLOW() { return 0; }
int __attribute__((noinline)) fun_FLOW_TO_EXP_BACKEND() { return 0; }
int __attribute__((noinline)) fun_FLOW_TO_GOOD_BACKEND() { return 0; }
int __attribute__((noinline)) fun_NEW_BACKEND() { return 0; }
int __attribute__((noinline)) fun_NEW_BACKEND_INSERTED() { return 0; }
int __attribute__((noinline)) fun_REJ_BACKEND() { return 0; }
int __attribute__((noinline)) fun_DROP_BACKEND() { return 0; }
