...
bool lb_backend_condition(void *key, void *state) {
  return 0 < ((struct LoadBalancedBackend *)key)
                 ...
                 ->nic < rte_eth_dev_count_avail();
}

bool lb_flow_id2backend_id_cond(void *key, void *state) {
  return 0 <= *(uint32_t *)key AND * (uint32_t *)key <
...
struct LoadBalancer *lb_allocate_balancer(uint32_t flow_capacity,
  ...
  if (map_allocate(lb_flow_equality, lb_flow_hash, flow_capacity,
                   &(balancer->flow_to_flow_id)) == 0) {
    goto err;
  }

  if (vector_allocate(sizeof(struct LoadBalancedFlow), flow_capacity,
                      lb_flow_init, &(balancer->flow_heap)) == 0) {
    goto err;
  }

  if (vector_allocate(sizeof(uint32_t), flow_capacity, null_init,
                      &(balancer->flow_id_to_backend_id)) == 0) {
    goto err;
  }
  ...
  if (vector_allocate(sizeof(uint32_t), backend_capacity, null_init,
                      &(balancer->backend_ips)) == 0) {
    goto err;
  }

  if (vector_allocate(sizeof(struct LoadBalancedBackend), backend_capacity,
                      lb_backend_init, &(balancer->backends)) == 0) {
    goto err;
  }

  if (map_allocate(lb_ip_equality, lb_ip_hash, backend_capacity,
                   &(balancer->ip_to_backend_id)) == 0) {
    goto err;
  }
  ...
  if (vector_allocate(sizeof(uint32_t), cht_height * backend_capacity,
                      null_init, &(balancer->cht)) == 0) {
    goto err;
  }

#ifdef KLEE_VERIFICATION
  available_backends =
      klee_range(0, backend_capacity + 1, "available_backends");
  ...
  vector_set_entry_condition(balancer->flow_id_to_backend_id,
                             lb_flow_id2backend_id_cond, balancer);

  vector_set_entry_condition(balancer->backends, lb_backend_condition,
                             balancer);
  ...
  DS_INIT(map, balancer->flow_to_flow_id, "active_flows", "flow")
  DS_INIT(vector, balancer->flow_heap, "", "")
  DS_INIT(dchain, balancer->flow_chain, "active_flows", "flow")
  DS_INIT(vector, balancer->flow_id_to_backend_id, "", "")
  DS_INIT(vector, balancer->backend_ips, "", "")
  DS_INIT(vector, balancer->backends, "", "")
  DS_INIT(map, balancer->ip_to_backend_id, "active_backends", "backend")
  DS_INIT(dchain, balancer->active_backends, "active_backends", "backend")
  DS_INIT(vector, balancer->cht, "", "")
  return balancer;
...
#ifdef KLEE_VERIFICATION
int lb_find_preferred_available_backend(struct LoadBalancedFlow *flow,
                                        struct Vector *cht,
                                        struct DoubleChain *active_backends,
                                        uint32_t cht_height,
                                        uint32_t backend_capacity,
  ...
  klee_trace_ret();
  /* To differentiate between multiple CHTs */
...
int lb_find_preferred_available_backend(struct LoadBalancedFlow *flow,
  ...
  uint64_t hash = (uint64_t)lb_flow_hash(flow);
  for (int i = 0; i < backend_capacity; ++i) {
    ...
    // printf("Looking for candidate %d:%d\n ", candidate_idx, *candidate);
...
struct LoadBalancedBackend lb_get_backend(struct LoadBalancer *balancer,
  ...
  if (map_get(balancer->flow_to_flow_id, flow, &flow_index) == 0) {
    int backend_index;
    int found = lb_find_preferred_available_backend(
        flow, balancer->cht, balancer->active_backends, balancer->cht_height,
        balancer->backend_capacity, &backend_index);
    if (found) {
      if (dchain_allocate_new_index(balancer->flow_chain, &flow_index, now) !=
          0) {
        ...
        vector_borrow(balancer->flow_id_to_backend_id, flow_index,
                      (void **)&vec_flow_id_to_backend_id);
        *vec_flow_id_to_backend_id = backend_index;
        vector_return(balancer->flow_id_to_backend_id, flow_index,
                      (void *)vec_flow_id_to_backend_id);
        map_put(balancer->flow_to_flow_id, vec_flow, flow_index);
        vector_return(balancer->flow_heap, flow_index,
                      vec_flow); // other half in map
      ...
      // Drop
      // VIGOR_TAG(TRAFFIC_CLASS, DROPPED_CLIENT_REQUEST);
    ...
    vector_return(balancer->flow_id_to_backend_id, flow_index,
                  (void *)vec_backend_index);
    if (0 ==
        dchain_is_index_allocated(balancer->active_backends, backend_index)) {
  ...
  klee_assert(backend.nic < rte_eth_dev_count_avail());
  for (uint16_t n = 0; n < rte_eth_dev_count_avail(); n++)
    if (backend.nic == n) {
      backend.nic = n;
      break;
    }
...
void lb_process_heartbit(struct LoadBalancer *balancer,
  ...
  if (map_get(balancer->ip_to_backend_id, &flow->src_ip, &backend_index) == 0) {
    if (0 != dchain_allocate_new_index(balancer->active_backends,
                                       &backend_index, now)) {
      // printf("allocated new backend %d \n", backend_index);
    ...
    // assert(dchain_is_index_allocated(balancer->active_backends,
    // backend_index));
...
void lb_expire_flows(struct LoadBalancer *balancer, time_t now) {
  ...
  uint64_t last_time_u =
      now_u -
      balancer
          ->flow_expiration_time; // OK because now >= flow_expiration_time >= 0
  time_t last_time = (time_t)last_time_u; // OK since the assert above passed

  expire_items_single_map(balancer->flow_chain, balancer->flow_heap,
                          balancer->flow_to_flow_id, last_time);
}
...
void lb_expire_backends(struct LoadBalancer *balancer, time_t now) {
  ...
  uint64_t now_u = (uint64_t)now; // OK since assert above passed and now > 0
  uint64_t last_time_u =
      now_u - balancer->backend_expiration_time; // OK because now >=
                                                 // flow_expiration_time >= 0
  time_t last_time = (time_t)last_time_u; // OK since the assert above passed

  expire_items_single_map(balancer->active_backends, balancer->backend_ips,
                          balancer->ip_to_backend_id, last_time);
}

#ifdef KLEE_VERIFICATION

__attribute__((noinline)) struct Map **
lb_get_flow_to_flow_id(struct LoadBalancer *balancer) {
  return &(balancer->flow_to_flow_id);
}
__attribute__((noinline)) struct Vector **
lb_get_flow_heap(struct LoadBalancer *balancer) {
  return &(balancer->flow_heap);
}
__attribute__((noinline)) struct DoubleChain **
lb_get_flow_chain(struct LoadBalancer *balancer) {
  return &(balancer->flow_chain);
}
__attribute__((noinline)) struct Vector **
lb_get_flow_id_to_backend_id(struct LoadBalancer *balancer) {
  return &(balancer->flow_id_to_backend_id);
}
__attribute__((noinline)) struct Vector **
lb_get_backend_ips(struct LoadBalancer *balancer) {
  return &(balancer->backend_ips);
}
__attribute__((noinline)) struct Vector **
lb_get_backends(struct LoadBalancer *balancer) {
  return &(balancer->backends);
}
__attribute__((noinline)) struct Map **
lb_get_ip_to_backend_id(struct LoadBalancer *balancer) {
  return &(balancer->ip_to_backend_id);
}
__attribute__((noinline)) struct DoubleChain **
lb_get_active_backends(struct LoadBalancer *balancer) {
  return &(balancer->active_backends);
}
__attribute__((noinline)) struct Vector **
lb_get_cht(struct LoadBalancer *balancer) {
  return &(balancer->cht);
}
#endif // KLEE_VERIFICATION
