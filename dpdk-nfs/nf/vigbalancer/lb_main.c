...
int nf_core_process(struct rte_mbuf *mbuf, time_t now) {
  ...
  if (mbuf->port != 0) {
    ...
    // VIGOR_TAG(TRAFFIC_CLASS, HEARTBEAT);
    lb_process_heartbit(balancer, &flow, ether_header->s_addr, mbuf->port, now);
    return mbuf->port;
  }
  // VIGOR_TAG(TRAFFIC_CLASS, CLIENT_REQUEST);
...
void nf_config_cmdline_print_usage(void) { lb_config_cmdline_print_usage(); }

void nf_print_config() { lb_print_config(&config); }

#ifdef KLEE_VERIFICATION
#include "lb_loop.h"

void nf_loop_iteration_border(unsigned lcore_id, time_t time) {
  lb_loop_iteration_border(
      lb_get_flow_to_flow_id(balancer), lb_get_flow_heap(balancer),
      lb_get_flow_chain(balancer), lb_get_flow_id_to_backend_id(balancer),
      lb_get_backend_ips(balancer), lb_get_backends(balancer),
      lb_get_ip_to_backend_id(balancer), lb_get_active_backends(balancer),
      lb_get_cht(balancer), time, config.backend_capacity,
